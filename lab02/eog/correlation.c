                    ...
                    int delka_signalu, double *jadro, int delka_jadra)
{
 ...
 // dopleno v predchozi uloze

}

void korelace (double *input, double *output, int len_sig, double *kernel, int len_kernel) {

    double mean = stredni_hodnota(kernel, len_kernel);

    int shift = (len_kernel - 1) / 2;
    for (int i=shift; i<len_sig; i++) {
        output[i] = 0.0;
        for (int j=len_kernel-1; j>=0; j--) {
            if ((i-j) > 0) {
                output[i-shift] += (input[i-j] * (kernel[j]-mean));
            } else {
                output[i-shift] += (input[0] * (kernel[j]-mean));
            }
        }
  }
